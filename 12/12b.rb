...
starting_row = file.index { |row| row.include?("E") }
starting_col = file[starting_row].index { |col| col == "E" }
    ...
    starting_points.each do |x_point, y_point|
        ...
        dirs =
            [
                ...
                .filter do |x, y|
                    -1 * (grid[y][x][:height] - grid[y_point][x_point][:height]) < 2
                end
...
puts solve(mapped, [[starting_col, starting_row]], "a") - 1
